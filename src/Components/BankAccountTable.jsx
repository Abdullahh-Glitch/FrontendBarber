...
const BankAccountTable = ({ accounts, isLoading }) => {
      ...
      <div className="overflow-auto thin-scrollbar bg-gradient-to-r from-[var(--table--from)] to-[var(--table--to)] rounded-t-2xl h-full">
        <table className="w-full border border-border">
          <thead className="bg-gradient-to-b from-[var(--table--header--from)] to-[var(--table--header--to)] sticky top-0 z-10">
            <tr >
              <th className="py-1 w-[2%] text-center text-xs font-bold text-muted-foreground uppercase tracking-wider">
                Sr
              </th>
              ...
              <th className="px-3 py-3 w-[10%] text-left text-xs font-bold text-muted-foreground uppercase tracking-wider">
                Branch Code
              </th>
          ...
          <tbody className="divide-y divide-border border border-border">
            {accounts.map((account, index) => (
              <tr key={index} className={`hover:bg-[var(--row--hover)] transition-colors border border-[var(--border-color)] border-border  duration-100 ${index % 2 === 0 ? 'bg-gradient-to-l from-[var(--row--e--from)] to-[var(--row--e--to)]' : 'bg-gradient-to-b from-[var(--row--o--from)] to-[var(--row--o--to)]'} h-[4px]`}>
                ...
                <td className="px-3 border-r font-semibold border-border border-[var(--border-color)]">
                  {account.bankName}
                </td>
                <td className="px-3 text-left font-semibold border-r border-border border-[var(--border-color)]">
                  {account.accountPersonName}
                </td>
                <td className="px-3 text-left font-bold border-r border-border border-[var(--border-color)] text-sm text-foreground">
                  {account.accountNumber}
                </td>
                <td className="px-3 font-bold border-r border-border border-[var(--border-color)] text-sm text-foreground text-left">
                  {account.IBANNo}
                </td>
                <td className="px-3 font-bold border-r border-border border-[var(--border-color)] text-sm text-foreground text-left">
                  {account.branchCode}
