...
const SalesInvoicePage = () => {
  ...
  const updateQty = (id, delta) => {
    setItems(
      items.map((i) =>
        i.id === id ? { ...i, qty: Math.max(1, i.qty + delta), totalAmount: calculateItemTotal(Math.max(1, i.qty + delta), i.price) } : i
      )
    );
  };
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg">
            ...
            <div className="flex flex-row gap-2 overflow-x-auto thin-scrollbar p-1 pt-2 bg-gradient-to-t from-[var(--secondary-from)] to-[var(--secondary-to)] rounded-b-lg">
              {serviceData?.length > 0 && serviceData?.map((s) => (
                <ItemCard key={s.name} data={s} onAdd={addItem} dataType="service" />
              ))}
            </div>
          </div>
          ...
          <div className="rounded-lg">
            ...
            <div className="flex flex-row gap-2 overflow-x-auto thin-scrollbar p-1 bg-gradient-to-t from-[var(--secondary-from)] to-[var(--secondary-to)] rounded-b-lg"> 
              {productData?.length > 0 && productData?.map((p) => (
                <ItemCard key={p.id} data={p} onAdd={addItem} dataType="product" />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="border bg-gradient-to-b from-[var(--saleItem--from)] to-[var(--saleItem--to)] rounded-lg p-2">
            <div className="flex flex-col md:flex-row gap-2 md:gap-0 justify-between align-middle">
              ...
              <div className="w-full md:w-[25%] flex items-center gap-7 md:gap-2 mr-2">
                <label htmlFor="invoiceDate" className="font-semibold">Date:</label>
                <input
                type="date"
                name="invoiceDate"
                value={edate}
            ...
            <div className="w-full h-[50vh] overflow-auto thin-scrollbar bg-white/40 rounded p-1">
              ...
              <tr key={index} className={`${index % 2 === 0 ? 'bg-[oklch(95%_0.052_163.051)]' : 'bg-[oklch(90.5%_0.093_164.15)]'} w-full border`}>
                <td className="w-[5%] text-center border-x-1 border-b-1">{index + 1}</td>
                <td className="text-left font-bold pl-2 border-r-1 border-b-1">{item.name}</td>
                ...
                <td className="w-[18%] text-right pr-1 border-r-1 border-b-1">
                  ...
                  type="number"
                  disabled
                  value={calculateItemTotal(item.qty, item.price).toFixed(2)}
            ...
            <div className="w-full md:w-[50%] flex flex-col font-bold px-1 bg-white/40 rounded">
              ...
              <div>
                ...
                <div className="flex flex-row justify-end">
                  ...
                  className="text-right w-[80%] bg-gray-400"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-row justify-between py-1">
                ...
                <div className="flex flex-row justify-end">
                  ...
                  // value={item.price ? item.price.toFixed(2) : 0}
                  // onChange={(e) => handleChange(item.id, "price", e.target.value)}
                  className="text-right w-[80%] bg-gray-300"
                  />
                </div>
              </div>
              ...
              <div>
                  <span className="text-[13px] md:text-[17px]">Total Received</span>
                </div>
                <div className="flex flex-row justify-end">
                  <input
                  type="text"
